...
import { Plus, HeartPulse, AlertTriangle, FileText, Edit, Trash2, Eye, HeartCrackIcon } from "lucide-react";

import * as healthService from "../services/health";

export default function Health() {
  const [dialogOpen, setDialogOpen] = React.useState(false);
  const [editingNote, setEditingNote] = React.useState<healthService.HealthNote | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);
  const [noteToDelete, setNoteToDelete] = React.useState<healthService.HealthNote | null>(null);
  const [viewNotesDialog, setViewNotesDialog] = React.useState(false);
  const [selectedChild, setSelectedChild] = React.useState<healthService.HealthRecord | null>(null);

  const [rows, setRows] = React.useState<healthService.HealthRecord[]>([]);
  const [childrenOptions, setChildrenOptions] = React.useState<{ id: string; child: string }[]>([]);
  ...
  // Fetch all health records and children list
  const fetchData = async () => {
    try {
      setLoading(true);
      const [healthData, children] = await Promise.all([
        healthService.fetchHealthRecords(),
        healthService.fetchChildren(),
      ]);
      setRows(healthData);
  ...
  const openEditDialog = (note: healthService.HealthNote) => {
    setEditingNote(note);
    setFormData({
      child: note.child,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingNote) {
        await healthService.updateHealthNote(editingNote.id, formData);
      } else {
        const selectedHealth = rows.find((r) => r.child === formData.child);
        if (!selectedHealth) {
          alert("Please select a valid child!");
          return;
        }
        await healthService.createHealthNote({
          ...formData,
          healthId: selectedHealth.id,
          id: "", // assuming backend will generate the ID
        });
      }
      await fetchData();
      resetForm();
  ...
  const handleDeleteNote = (note: healthService.HealthNote) => {
  ...
  const confirmDelete = async () => {
    if (noteToDelete) {
      try {
        await healthService.deleteHealthNote(noteToDelete.id);
        await fetchData();
  ...
  const viewAllNotes = (child: healthService.HealthRecord) => {
  ...
  return (
    <AppShell title="Health Records">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-pink-500 to-pink-400 opacity-90" />
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-white/90">
              Allergies, immunizations, and emergency information.
            </p>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            <DialogBody className="space-y-4">
              ...
              <div>
                ...
                >
                  <SelectTrigger className="mt-1">{formData.child || "Select child"}</SelectTrigger>
                  <SelectContent>
                    {childrenOptions.map((c) => (
                      <SelectItem key={c.id} value={c.child}>
                        {c.child}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div>
                ...
                >
                  <SelectTrigger className="mt-1">{formData.noteType || "Select type"}</SelectTrigger>
                  <SelectContent>
